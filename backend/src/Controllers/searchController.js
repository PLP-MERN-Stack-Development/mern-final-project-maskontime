    ...
    try {
        ...
        if (searchType === 'all' || searchType === 'workouts') {
            results.workouts = await Workout.find({
              ...
              .select('name description image category difficulty duration')
        ...
        if (searchType === 'all' || searchType === 'personnel') {
            results.personnel = await Personnel.find({
              ...
              .select('name role specialization rating isVerified profileImage')
              .select('-password');
        }
    ...
    try {
        ...
        if (searchTypes.includes('meals')) {
            ...
            if (filters?.meals) {
                ...
                if (filters.meals.healthBenefits) mealQuery.healthBenefits = { $regex: filters.meals.healthBenefits, $options: 'i' };
            }
        ...
        if (searchTypes.includes('personnel')) {
            ...
            if (filters?.personnel) {
                ...
                if (filters.personnel.isVerified !== undefined) personnelQuery.isVerified = filters.personnel.isVerified;
            }
