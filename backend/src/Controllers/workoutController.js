...
exports.getWorkouts = async (req, res, next) => {
    try {
        ...
        const workouts = await Workout.find(query)
            .populate({ path: 'trainer', select: 'name specialization rating' })
...
exports.updateWorkout = async (req, res, next) => {
    try {
        ...
        if (req.user && workout.trainer && workout.trainer.toString() !== req.user.id && req.user.role !== 'admin') {
            return res.status(403).json({ success: false, error: 'Not authorized to update this workout' });
        }
...
exports.deleteWorkout = async (req, res, next) => {
    try {
        ...
        // Check ownership
        if (req.user && workout.trainer && workout.trainer.toString() !== req.user.id && req.user.role !== 'admin') {
            return res.status(403).json({ success: false, error: 'Not authorized to delete this workout' });
        }
...
exports.addWorkoutReview = async (req, res, next) => {
    try {
        ...
        if (alreadyReviewed) return res.status(400).json({ success: false, error: 'Workout already reviewed' });
